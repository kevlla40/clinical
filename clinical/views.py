...
# Sidebar click view for admin page
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
# Delete a doctor from the hospital
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
# Delete a nurse from the hospital
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
...
@user_passes_test(is_admin)
def delete_nurse_from_hospital_view(request,pk):
    nurses=models.Nurse.objects.get(id=pk)
    user=models.User.objects.get(id=nurses.user_id)
    user.delete()
    nurses.delete()
    return redirect('admin-view-nurse')
...
def doctor_patient_view(request):
    ...
    return render(request, 'hospital/doctor_patient.html', context=context)
...
def search_view(request):
    ...
    patients = models.Patient.objects.filter(status=True, assignedDoctorId=request.user.id).filter(Q(symptoms__icontains=query) | Q(user__first_name__icontains=query))
    return render(request, 'hospital/doctor_view_patient.html', {'patients': patients, 'doctor': doctor})
...
def nurse_dashboard_view(request):
    # For three cards
    patientcount = models.Patient.objects.filter(status=True, assignedNurse_id=request.user.id).count()  # Corrected field name
    appointmentcount = models.Appointment.objects.filter(status=True, nurse_id=request.user.id).count()  # Assuming nurseId exists in the model
    patientdischarged = models.PatientDischargeDetails.objects.filter(assignedNurseName=request.user.first_name).distinct().count()

    # For table in nurse dashboard
    appointments = models.Appointment.objects.filter(status=True, nurse_id=request.user.id).order_by('-id')  # Assuming nurseId exists in the model
    patient_ids = [a.patientId for a in appointments]
    patients = models.Patient.objects.filter(status=True, user_id__in=patient_ids).order_by('-id')
    appointments = zip(appointments, patients)
    ...
    return render(request, 'hospital/nurse_dashboard.html', context=context)
...
def nurse_patient_view(request):
    ...
    return render(request, 'hospital/nurse_patient.html', context=context)


@login_required(login_url='nurselogin')
@user_passes_test(is_nurse)
def nurse_view_patient_view(request):
    patients = models.Patient.objects.filter(status=True, assignedNurse_id=request.user.id)  # Corrected field name
    nurse = models.Nurse.objects.get(user_id=request.user.id)  # For profile picture of nurse in sidebar
...
def search_view(request):
    nurse = models.Nurse.objects.get(user_id=request.user.id)  # For profile picture of nurse in sidebar
    query = request.GET.get('query')
    patients = models.Patient.objects.filter(status=True, assignedNurse_id=request.user.id).filter(Q(symptoms__icontains=query) | Q(user__first_name__icontains=query))  # Corrected field name
    return render(request, 'hospital/nurse_view_patient.html', {'patients': patients, 'nurse': nurse})
...
def nurse_view_discharge_patient_view(request):
    dischargedpatients = models.PatientDischargeDetails.objects.filter(assignedNurseName=request.user.first_name).distinct()
    nurse = models.Nurse.objects.get(user_id=request.user.id)  # For profile picture of nurse in sidebar
...
def nurse_view_appointment_view(request):
    nurse = models.Nurse.objects.get(user_id=request.user.id)  # For profile picture of nurse in sidebar
    appointments = models.Appointment.objects.filter(status=True, nurse_id=request.user.id)  # Assuming nurseId exists in the model
    patient_ids = [a.patientId for a in appointments]
    patients = models.Patient.objects.filter(status=True, user_id__in=patient_ids)
    appointments = zip(appointments, patients)
...
def nurse_delete_appointment_view(request):
    nurse = models.Nurse.objects.get(user_id=request.user.id)  # For profile picture of nurse in sidebar
    appointments = models.Appointment.objects.filter(status=True, nurse_id=request.user.id)
    patient_ids = [a.patientId for a in appointments]
    patients = models.Patient.objects.filter(status=True, user_id__in=patient_ids)
    appointments = zip(appointments, patients)
